...
impl ScanTask<FTPScanResult> for FTPScanTask {
    async fn scan<S: Send + Sync + AsyncRead + AsyncWrite + Unpin + 'static>(self, stream: &mut S) -> Result<FTPScanResult, SimpleError> {
        FTPScanTask::scan(stream).await
    }
